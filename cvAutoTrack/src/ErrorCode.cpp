...
{
    HKEY hKey;
    // 注册表路径，注意是CentralProcessor下的第一个处理器（0）
    ...
    // 查询ProcessorNameString的大小
    DWORD dataSize = 0;
    result = RegQueryValueExW(hKey, L"ProcessorNameString", nullptr, nullptr, nullptr, &dataSize);
    if (result != ERROR_SUCCESS) {
        RegCloseKey(hKey);
        return {};
    }
    ...
    char* u8_buffer = new char[dataSize * 4];
    std::string cpu_name{};
    result = RegQueryValueExW(hKey, L"ProcessorNameString", nullptr, nullptr, (LPBYTE)buffer, &dataSize);
    if (result == ERROR_SUCCESS) {
        WideCharToMultiByte(CP_UTF8, 0, buffer, dataSize / sizeof(wchar_t), u8_buffer, dataSize, nullptr, nullptr);
        cpu_name = u8_buffer;
    }
...
std::string get_cpu_baseline()
{
    //获取CPU支持的指令集功能，用来排查是否因为部分优化功能导致程序崩溃
    //cv::utils::logging::setLogLevel(cv::utils::logging::LOG_LEVEL_VERBOSE);
    std::string base_line = cv::getCPUFeaturesLine();
    return base_line;
}
...
inline void write_log(std::fstream& log_file, const std::string& time_stamp, const int error_code, const std::string& error_message)
{
    ...
    if (file_size > 1024 * 1024 * 2)
    {
        ...
        log_file << "CPU型号：" << get_cpu_name() << "\n";
        log_file << "支持的指令集：" << get_cpu_baseline() << "\n";
