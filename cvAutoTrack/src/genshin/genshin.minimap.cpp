...
bool match_minimap_cailb(const GenshinScreen& genshin_screen, GenshinIconSight& out_genshin_minimap_cailb)
{
    ...
    auto& giMinimapCailbRef = genshin_screen.imgs.icon_sight_maybe;
    auto& rect_origin = genshin_screen.rects.icon_sight_maybe;
    ...
    double check_match_minimap_cailb_param = out_genshin_minimap_cailb.config.check_match_icon_sight_params;
    if (genshin_screen.config.is_used_alpha == false)
    {
        cv::cvtColor(genshin_screen.imgs.icon_sight_maybe, giMinimapCailbRef, cv::COLOR_RGBA2GRAY);
        template_not_handle_mode = minimap_cailb_template_no_alpha;
        template_handle_mode = minimap_cailb_template_no_alpha_handle_mode;
        check_match_minimap_cailb_param = out_genshin_minimap_cailb.config.check_match_icon_sight_params;
    }
    ...
    else
    {
        ...
        if (is_handle_mode)
        {
            out_genshin_minimap_cailb.rect_Icon_sight = cv::Rect(rect_origin.tl() + minimap_cailb_match_maxLoc, template_handle_mode.size());
        }
        else
        {
            out_genshin_minimap_cailb.rect_Icon_sight = cv::Rect(rect_origin.tl() + minimap_cailb_match_maxLoc, template_not_handle_mode.size());
        }
    }
...
bool cailb_minimap_impl(const GenshinScreen& genshin_screen, GenshinMinimap& out_genshin_minimap)
{
    ...
    // rect
    auto& minimap_rect = genshin_screen.rects.minimap;
    if (genshin_screen.config.is_search_mode)
    {
        static GenshinIconSight genshin_minimap_cailb;

        //auto& minimap_cailb_rect = genshin_minimap_cailb.rect_Icon_sight;
